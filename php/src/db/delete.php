	...
	if ( get('company_id') ) {
		...
		if ( !get('job_id') ) {
			...
			if ( get('all') ) {
				...
				$query_tmp = $query['select_all_jobs_ids_with_company_id'] . get('company_id');
				$result_jobids = mysql_query($query_tmp, $db_connection);
				
				if($result_jobids) {
					while ($row = mysql_fetch_array($result_jobids, MYSQL_NUM)) {
						if(!mysql_query($query['delete_job_by_id'] . $row[0], $db_connection)) {
			...
			// delete the company
			$result = mysql_query( $query['delete_company_by_id'] . get('company_id'), $db_connection);
			
		} else {
			
			// delete the job ..
			$result = false;
			if(mysql_query( $query['delete_job_by_id'] . get('job_id'), $db_connection)) {
				// .. and update the company job counter
				$result = mysql_query( $query['update_jobs_minus_from_company_id'] . get('company_id'), $db_connection);
			}
